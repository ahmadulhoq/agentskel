#!/usr/bin/env bash
# agentskel pre-memory-push enforcement hook (Cursor variant)
...
print(json.dumps({
    'permission': 'deny',
    'user_message': 'Pre-push pull failed. Resolve conflicts in .memory/ before pushing.',
    'agent_message': '''Pre-push pull failed: ${PULL_OUTPUT}. Resolve conflicts in .memory/ before pushing.'''
}))
"
                    exit 0
                }
