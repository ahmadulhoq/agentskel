...
- **Fast Execution Mode** — when `Fast Execution Mode` in `.memory/CONFIG.md` is `on`, OR the user prefixed the current request with `fast:`, the agent skips the branch + PR ceremony and commits/pushes directly to the default branch. Plan-first, task-completion, validator, and memory updates still apply. Before any commit in this mode, the agent MUST surface a one-line banner like `FAST MODE ACTIVE — committing directly to <branch> (no PR).` so the user knows ceremony was skipped. Use sparingly: typo fixes, version markers, lockfile bumps. See the `git-flow` skill's "Fast Mode Bypass" section for the procedure.
