...
Steps:
1. Branch `release/vX.Y.Z` from `development`
...
```
release/v9.8.0 → PE-XXXX-fix-name → PR → release/v9.8.0
```
...
| Release | `development` | `release/vX.Y.Z` | `development` | Merge commit |
| Hotfix (release open) | `release/vX.Y.Z` | `PE-XXXX-description` | `release/vX.Y.Z` | Squash |
| Hotfix (after merge-back) | tagged commit | `release/vX.Y.Z+1` | `development` | Merge commit |
