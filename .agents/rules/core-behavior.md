...
- **Direct-Commit Mode** — when `Direct-Commit Mode` in `.memory/CONFIG.md` is `on`, OR the user prefixed the current request with `direct:`, the agent skips the branch + PR ceremony and commits/pushes directly to the default branch. Plan-first, task-completion, validator, and memory updates still apply. Before any commit in this mode, the agent MUST surface a one-line banner like `DIRECT-COMMIT MODE ACTIVE — committing directly to <branch> (no PR).` so the user knows ceremony was skipped. Use sparingly: typo fixes, version markers, lockfile bumps. See `docs/AUTONOMY-MODES.md` for the full procedure.
- **Autopilot Mode** — when `Autopilot Mode` in `.memory/CONFIG.md` is `on`, the harness (Claude Code / Cursor / etc.) auto-approves safe operations — reads, project-scoped writes, non-destructive git/gh commands — so the agent isn't interrupted for trivial permission prompts once a plan is approved. Enforced via expanded `permissions.allow` in `settings.json` + `pre-bash-safety.sh` hook which blocks destructive patterns regardless of allowlist. **Does NOT bypass plan approval, decisions, or concerns raised** — those workflow-level gates stay strict. **Still pauses for:** destructive ops (rm -rf, git push --force, git reset --hard, branch -D, git checkout . or `-- <file>`, git clean -f, worktree remove --force), out-of-project paths, dependency upgrades, sacred behaviors, and (behaviorally, per the v1.65.0 significant-change gate) >30 lines of existing logic / public API / documented-behavior removal. Persistent toggle only — no one-shot prefix. Composable with Direct-Commit Mode (independent toggles). `session-start` surfaces a banner when the mode is on. See `docs/AUTONOMY-MODES.md` for boundary definitions and mode combinations.
- **Present each PR URL on its own line.** When a turn opens multiple PRs (rare but happens), list each as `PR #N: <url>` on its own line in the end-of-turn summary — not comma-separated inline — so the user can click each one independently.
- **Post-merge cleanup is mandatory after the user confirms a merge.** When the user says "merged" (or equivalent), run the `git-flow` Post-Merge Cleanup procedure **before any other work**: switch to default branch, pull, delete the local feature branch, prune stale remote refs, update RESUME. Only then proceed to the next task.
- **Honor user-specified commit granularity** for the duration of the workflow. If the user says "smaller commits", "atomic commits", "commit per file", "commit per logical unit", or equivalent, apply that granularity to every commit until the workflow ends — it is **not** a one-shot instruction. Surface the chosen granularity in the workflow plan so it stays visible across steps. Default when no instruction given: one commit per logical change.
