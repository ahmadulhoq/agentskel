...
| `scripts/` | Developer onboarding + validation | install-agent.sh (mount memory worktree + link external skills); validate.py (12 deterministic checks: frontmatter, descriptions, version consistency across 5 files, parity for 5 tool stub dirs (claude, gemini, cursor, windsurf, copilot), AGENTS.md catalog parity, inline-rule propagation, CHANGELOG presence, and — new in v1.67.1 — no unreleased skeleton changes, which diffs core/roles/scripts/.agents against the last VERSION-touching commit and fails on shallow clones rather than passing on an empty diff) |
