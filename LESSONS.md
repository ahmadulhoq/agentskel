...
## Lesson 009 — 2026-09-22
- **Mistake:** PR #56 changed `core/claude-hooks/` and merged to `main` with none of the release protocol run — no VERSION bump, no CHANGELOG entry, no version markers, no downstream migration step. The gap sat undetected until the user asked for an audit. PR #55 (v1.67.0) had separately skipped the memory-side half (`.memory/CHANGELOG.md`, `TIME_LOG.md`).
- **Pattern:** A PR framed as "a fix" rather than "a release" doesn't feel like it needs release ceremony, and nothing contradicted that feeling. Worse, the PR body asserted downstream projects would "pick this up on their next sync" — an assumption never checked against `sync-skeleton`, where Step 5k's `Skip if already on v1.66.1+` guard meant the affected projects were exactly the ones that would skip the fix. A plausible-sounding delivery claim substituted for reading the delivery path.
- **Rule:** Two checks, both mechanical. (1) If a diff touches `core/`, `roles/`, `scripts/`, or `.agents/`, it is a release — run the skeleton-contribution checklist before merge, not after. This is now enforced by the `no unreleased skeleton changes` validator check, so it fails CI rather than relying on memory. (2) Never claim downstream will receive a change without opening `sync-skeleton` and finding the step that delivers it. A migration step whose skip-guard excludes the affected population delivers nothing; for a fix to a shipped defect, the guard must be inverted to target the versions that *have* the defect.
- **Meta-lesson on green CI:** `498 ok, 0 fail` was true and meaningless. `version consistency` only compared the five markers to each other and `changelog has current version entry` only asserted the CHANGELOG matched whatever VERSION said — both self-referential, both passing on a repo with an unversioned change in it. When a defect class gets through, check whether the validator *could* have expressed it before assuming it was a human lapse.
