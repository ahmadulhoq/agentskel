...
> Corresponds to: agentskel v1.59.2
...
11. **Deterministic enforcement via hooks**: Critical artifacts (CHANGELOG, TIME_LOG) are enforced by PreToolUse hooks that block commits without them. PreToolUse hooks also auto-pull before ai-memory pushes. Stop hooks verify task-completion before session end. An advisory PreToolUse hook fires before every Edit and Write call, reminding the agent to verify explicit approval before modifying files (exits 0 — non-blocking, surfaces the check at the tool boundary). These mechanisms close the enforcement gap: commit hooks cover artifacts, stop-verify covers uncommitted work, and the edit/write hook reinforces the plan-first gate at the exact moment a file is about to change.
