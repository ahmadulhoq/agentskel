...
- **Direct-Commit Mode**: when `Direct-Commit Mode` in `.memory/CONFIG.md` is `on`, or the user prefixed the request with `direct:`, skip the branch + PR ceremony and commit/push directly to the default branch. Plan-first + task-completion still apply. Surface a `DIRECT-COMMIT MODE ACTIVE — committing directly to <branch> (no PR).` banner before any commit.
- **Autopilot Mode**: when on, harness auto-approves safe operations (reads, project writes, non-destructive git/gh) via expanded `permissions.allow` + `pre-bash-safety.sh` hook. Does NOT bypass plan/decisions/concerns. Still pauses for destructive ops (--force, --hard, branch -D, checkout . or -- file, clean -f, recursive force delete), out-of-project paths, dependency upgrades, sacred behaviors, significant changes (per v1.65.0 gate). Persistent toggle. See `docs/AUTONOMY-MODES.md`.
