...
- **Autopilot Mode**: when `Autopilot Mode` in `.memory/CONFIG.md` is `on`, after a plan has been approved the agent proceeds within the plan's scope without per-step approval prompts. Always pauses for significant changes (>30 lines of existing logic / public API / sacred / documented-behavior removal), destructive ops, out-of-project paths, dependency changes, scope deviations. Persistent toggle only. See `docs/AUTONOMY-MODES.md`.
